...
class BasePoseBody:
    ...
    def frame_dropout(self, dropout_std: float):
        ...
        return self.select_frames([i for i in range(0, data_len) if i not in dropout_indexes])


class TorchPoseBody(BasePoseBody):
    def __init__(self, fps: int, data: MaskedTensor, confidence: torch.Tensor):
...
class PoseBody(BasePoseBody):
    def __init__(self, fps: int, data: ma.MaskedArray, confidence: np.ndarray):
        super().__init__(fps, data, confidence)

    @staticmethod
    def read_v0_0(header: PoseHeader, reader: BufferReader):
    ...
    @staticmethod
    def read_v0_1(header: PoseHeader, reader: BufferReader):
        fps, _frames = reader.unpack(ConstStructs.double_ushort)
    ...
    def read(header: PoseHeader, reader: BufferReader):
        if header.version == 0:
            return PoseBody.read_v0_0(header, reader)
        elif round(header.version, 3) == 0.1:
            return PoseBody.read_v0_1(header, reader)
    ...
    def torch(self):
        confidence = torch.from_numpy(self.confidence)
        masked_tensor = MaskedTensor(torch.from_numpy(self.data), torch.stack([confidence > 0] * 2, dim=3))
        return TorchPoseBody(self.fps, masked_tensor, confidence)
    ...
    def matmul(self, matrix: np.ndarray):
        ...
        return PoseBody(self.fps, data, self.confidence)
    ...
    def get_points(self, indexes: List[int]):
        ...
        return PoseBody(self.fps, new_data, new_confidence)
    ...
    def interpolate(self, new_fps: int, kind='cubic'):
        ...
        return PoseBody(fps=new_fps, data=masked_data, confidence=confidence)
