...
from pose_format.utils.generic import fake_pose, detect_known_pose_format, get_component_names, get_standard_components_for_known_format, SupportedPoseFormat, pose_hide_legs, pose_shoulders, hands_indexes, normalize_pose_size, pose_normalization_info, get_hand_wrist_index, get_body_hand_wrist_index, correct_wrist, correct_wrists, hands_components

import pytest
from typing import List

# Assuming SupportedPoseFormat, Pose, and fake_pose are defined elsewhere
...
@pytest.mark.parametrize(
    ...
    [(fmt, fmt) for fmt in get_args(SupportedPoseFormat)], 
    indirect=["fake_poses"]
)
def test_detect_format(fake_poses, expected_type):
    for fake_pose in fake_poses:
        detected_format = detect_known_pose_format(get_component_names(fake_pose))
        assert detected_format == expected_type

    with pytest.raises(ValueError, match="Could not detect pose format, unknown pose header schema with component names:"):
...
def test_get_component_names(fake_poses: List[Pose], format:SupportedPoseFormat):
    
    standard_components_for_format = get_standard_components_for_known_format(format)
...
@pytest.mark.parametrize(
    "fake_poses", 
    [fmt for fmt in get_args(SupportedPoseFormat)], 
    indirect=["fake_poses"]
)
...
def test_correct_wrists(fake_poses: List[Pose]):
    for pose in fake_poses:
        ...
        assert np.array_equal(corrected_pose.body.data, pose.body.data) == False
